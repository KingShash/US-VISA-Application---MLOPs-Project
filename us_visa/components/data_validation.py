...
class DataValidation:
    ...
    def detect_dataset_drift(self, reference_df: DataFrame, current_df: DataFrame, ) -> bool:
        ...
        try:
            ...
            reference_df = reference_df.drop(columns=[c for c in drop_cols if c in reference_df.columns])
            current_df = current_df.drop(columns=[c for c in drop_cols if c in current_df.columns])

            # Pandas 2.x uses ArrowStringArray for str columns; Evidently 0.2.8 expects np.ndarray
            ...
            dataset_drift_result = next(
                m["result"] for m in json_report["metrics"] if m["metric"] == "DatasetDriftMetric"
            )
            n_features = dataset_drift_result["number_of_columns"]
            n_drifted_features = dataset_drift_result["number_of_drifted_columns"]
